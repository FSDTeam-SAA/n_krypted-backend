...
exports.replaceInlineImagesWithCloudinary = replaceInlineImagesWithCloudinary;
exports.htmlToTextFallback = htmlToTextFallback;
// utils/inlineImages.ts
const cloudinary_1 = __importDefault(require("../utils/cloudinary"));
const buffer_1 = require("buffer");
const DATA_IMG_RE = /<img\b[^>]*\bsrc=["'](data:image\/[a-zA-Z0-9.+-]+;base64,([A-Za-z0-9+/=]+))["'][^>]*>/gi;
/**
 * Upload a Buffer to Cloudinary and return secure_url
 */
function uploadBufferToCloudinary(buf) {
...
async function replaceInlineImagesWithCloudinary(html) {
    ...
    if (matches.length === 0)
        return html;
    // Upload all images
    const uploads = await Promise.all(matches.map(async (m) => {
        const base64Payload = m[2]; // group2 is the raw base64 per regex
        const buf = buffer_1.Buffer.from(base64Payload, "base64");
    ...
    for (const { fullTag, url } of uploads) {
        if (!url)
            continue;
        const newTag = fullTag.replace(/src=["'][^"']+["']/, `src="${url}" style="max-width:100%;height:auto;display:block;margin:0 auto;border-radius:8px;"`);
