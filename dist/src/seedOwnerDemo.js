...
const demoDishes = [
    {
        name: "Schnitzel",
        description: "Goldbraun paniertes Schnitzel mit knusprigen Pommes frites und frischer Zitrone.",
        price: 18.9,
        image: "https://images.unsplash.com/photo-1599921841143-819065a55cc6?w=1000&auto=format&fit=crop&q=85",
        images: [
        ...
        preparationProcess: "Frisch paniert und in Butterschmalz goldbraun ausgebacken.",
        isSignatureDish: true,
        isActive: true,
    },
    {
        name: "Gebratenes Steak",
        description: "Saftig gebratenes Rindersteak mit Kräuterbutter, Tomaten und frischen Kräutern.",
        price: 26.5,
        ...
        preparationProcess: "Scharf angebraten, schonend fertig gegart und vor dem Servieren ruhen gelassen.",
        isSignatureDish: true,
        isActive: true,
    },
];
const demoVisits = [
    {
        ...
        distanceMeters: 18,
        rating: 4,
        comment: "Die goldbraune Kruste war leicht und knusprig, das Fleisch saftig und geschmackvoll. Sehr empfehlenswert.",
    },
    {
        name: "Lukas Weber",
        ...
        distanceMeters: 24,
        rating: 5,
        comment: "Das Steak war perfekt gegart, wunderbar saftig und die Kräuterbutter passte hervorragend dazu.",
    },
];
async function seedOwnerDemo() {
    ...
    try {
        const restaurant = await Deal_model_1.default.findOne({ title: targetRestaurantTitle });
        if (!restaurant) {
        ...
        const dishes = demoDishes.map((dish, index) => ({
            _id: previousDishes[index]?._id ?? new mongoose_1.default.Types.ObjectId(),
            ...dish,
            image: index === 0 && firstImage ? firstImage : dish.image,
            images: index === 0 && firstImage
        ...
        for (let index = 0; index < demoVisits.length; index += 1) {
            ...
            const checkedInAt = new Date(Date.now() - demo.daysAgo * 24 * 60 * 60 * 1000);
            const user = await User_model_1.default.findOneAndUpdate({ email: demo.email }, {
                name: demo.name,
                email: demo.email,
                avatar: demo.avatar,
                password,
            ...
            const checkIn = await CheckIn_model_1.default.findOneAndUpdate({ userId: user._id, restaurantId: restaurant._id }, {
                userId: user._id,
            ...
            await Review_model_1.default.findOneAndUpdate({ checkInID: checkIn._id }, {
                userID: user._id,
...
seedOwnerDemo().catch((error) => {
    console.error("Owner demo seed failed:", error.message);
    process.exitCode = 1;
});
